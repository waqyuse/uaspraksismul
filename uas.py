...
def generate_data(pixels, data):
    ...
    for a in range(length_of_data):
        pixels = [val for val in image_data.__next__(
        )[:3] + image_data.__next__()[:3] + image_data.__next__()[:3]]
...
def main_encryption(img, text, new_image_name):
    ...
    if (len(text) == 0) or (len(img) == 0) or (len(new_image_name) == 0):
        mb.showerror(
            "Error", 'Kamu belum menyimpan Nilai! Simpan semua nilai yang dibutuhkan sebelum menekan tombol!')
...
def main_decryption(img, strvar):
    ...
    while decoding:
        pixels = [value for value in image_data.__next__(
        )[:3] + image_data.__next__()[:3] + image_data.__next__()[:3]]
...
def encode_image():
    ...
    Label(encode_wn, text='Encode an Image', font=(
        "Comic Sans MS", 15), bg='AntiqueWhite').place(x=220, rely=0)
...
def decode_image():
    ...
    Label(decode_wn, text='Decode an Image', font=(
        "Comic Sans MS", 15), bg='Bisque').place(x=220, rely=0)
...
root = Tk()
root.title('UAS With GUI Image Steganography')
root.geometry('300x200')
root.resizable(0, 0)
root.config(bg='NavajoWhite')

Label(root, text='UAS With GUI Image Steganography', font=('Comic Sans MS', 15), bg='NavajoWhite',
